...
class DefaultParamAnalysisGraph(OptParamGraphs):
    ...
    def generate(self, lang: str = "pt-BR"):
        ...
        for param, values in self.results.items():
            ...
            filename = os.path.join(self.plots_folder, f"analysis-of-{param}.jpg")
    ...
    def generate_log(self, lang: str = "pt-BR"):
        ...
        for param, values in self.results.items():
            ...
            # print(ticks)
            # print(ticks[-1] + (ticks[-1] - ticks[-2]))
            # ticks = np.append(ticks, ticks[-1] + (ticks[-1] - ticks[-2]))
            # print(ticks)
            # print(" ")
            # sub1.set_yticks(ticks, minor=True)

            # sub1.set_ylim((sub1.get_ylim()[0], sub1.get_yticks()[-1]))
            # sub1.set_xticks(list(ax.get_xticks()) + extraticks)
            # yticks = sub1.get_yticks()
            # print(list(yticks))
            # np.append(yticks, (yticks[-1] - yticks[-2]))
            # sub1.set_yticks(yticks)
            ...
            filename = os.path.join(self.plots_folder, f"log_analysis-of-{param}.jpg")
            plt.savefig(filename)
            fig.clf()

    def generate_log2(self, lang: str = "pt-BR"):
        titles = self.get_titles(lang)
        yticks = []
        for param, values in self.results.items():
            labels = []
            ...
            width = 0.35
            fig, sub1 = plt.subplots(figsize=(10, 7))
            # fig = plt.figure(figsize=(10, 14))
            ...
            sub1.set_title(self.target_variable_display)
            sub1.set_ylabel(self.target_variable_display)
            sub1.bar_label(rec1, padding=1, fontsize=12, fontfamily="CMU Serif")
            yticks = sub1.get_yticks()
            np.append(yticks, (yticks[-1] - yticks[-2]))
            sub1.set_yticks(yticks)
            plt.yscale('log', nonpositive='clip')
